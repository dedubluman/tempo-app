...
export default function RecoverySettingsPage() {
  ...
  const handleRegisterBackup = async () => {
    ...
    try {
      ...
      await saveBackupCredentialMetadata(credentialId, targetAddress, "Backup device");
      ...
      const credentialHash = btoa(String.fromCharCode(...new Uint8Array(hash))).slice(0, 16);
      setSavedRecord({
        credentialHash,
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-8 sm:py-10">
        <section className="rounded-2xl border border-[--border-default] bg-[--bg-surface] p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm text-[--text-secondary]">
            Enable <code>NEXT_PUBLIC_FF_PASSKEY_RECOVERY=1</code> to use backup enrollment.
          </p>
        </section>
      </main>
    );
    ...
    return (
      <main className="mx-auto w-full max-w-3xl px-4 py-8 sm:py-10">
        <section className="rounded-2xl border border-[--status-warning-border] bg-[--status-warning-bg] p-5 sm:p-6 text-[--status-warning-text]">
          <p className="text-xs font-semibold uppercase tracking-[0.2em]">Passkey Recovery</p>
          <h1 className="mt-2 text-xl font-semibold">Connect your wallet first</h1>
          <p className="mt-2 text-sm">Backup enrollment requires an active session for your primary wallet.</p>
          <Link href="/" className="mt-4 inline-flex h-11 items-center rounded-xl border border-current px-4 text-sm font-semibold">
            Go to sign in
          </Link>
        </section>
      </main>
  ...
  return (
    <main className="mx-auto w-full max-w-3xl space-y-4 px-4 py-8 sm:py-10" data-testid="recovery-enrollment-page">
      ...
      <section className="rounded-2xl border border-[--border-default] bg-[--bg-surface] p-5 sm:p-6" data-testid="recovery-step-1">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[--text-tertiary]">Step 1</p>
        <h2 className="mt-2 text-lg font-semibold text-[--text-primary]">Why a backup matters</h2>
        <p className="mt-2 text-sm text-[--text-secondary]">
          If your primary device is lost and no backup passkey exists, you can permanently lose wallet access and funds.
        </p>
        {step === 1 ? (
          <button
      ...
      {step >= 2 ? (
        <section className="rounded-2xl border border-[--border-default] bg-[--bg-surface] p-5 sm:p-6" data-testid="recovery-step-2">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[--text-tertiary]">Step 2</p>
          <h2 className="mt-2 text-lg font-semibold text-[--text-primary]">Prepare your backup device</h2>
          <p className="mt-2 text-sm text-[--text-secondary]">
            Use a second trusted authenticator such as a laptop passkey or hardware key before continuing.
          </p>
          {step === 2 ? (
            <button
              className="mt-4 inline-flex h-11 items-center rounded-xl bg-[--brand-primary] px-4 text-sm font-semibold text-[--text-inverse]"
      ...
      {step >= 3 ? (
        <section className="rounded-2xl border border-[--border-default] bg-[--bg-surface] p-5 sm:p-6" data-testid="recovery-step-3">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[--text-tertiary]">Step 3</p>
          <h2 className="mt-2 text-lg font-semibold text-[--text-primary]">Register second passkey</h2>
          <p className="mt-2 text-sm text-[--text-secondary]">
            This uses the <span className="font-mono">webAuthn</span> connector and must resolve to your current wallet address.
          </p>
          <button
          ...
          >
            {isRegistering ? "Waiting for backup passkey..." : "Register Backup Passkey"}
          </button>
          {error ? (
      ...
      {step >= 4 ? (
        <section className="rounded-2xl border border-[--border-default] bg-[--bg-surface] p-5 sm:p-6" data-testid="recovery-step-4">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[--text-tertiary]">Step 4</p>
          <h2 className="mt-2 text-lg font-semibold text-[--text-primary]">Backup metadata stored</h2>
          <p className="mt-2 text-sm text-[--text-secondary]">
            Backup credential metadata is saved in <span className="font-mono">passkeyRegistry</span> for this account.
          </p>
      ...
      {step >= 5 ? (
        <section className="rounded-2xl border border-[--status-success-border] bg-[--status-success-bg] p-5 sm:p-6" data-testid="recovery-step-5">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[--status-success-text]">Step 5</p>
          <h2 className="mt-2 text-lg font-semibold text-[--status-success-text]">Backup setup complete</h2>
          <p className="mt-2 text-sm text-[--status-success-text]">
            You can now test recovery by opening recovery mode on a device without the primary credential.
          </p>
      ...
      <section className="rounded-2xl border border-[--border-default] bg-[--bg-surface] p-5 sm:p-6" data-testid="existing-backup-list">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[--text-tertiary]">Registered backups</p>
        {existingBackups.length === 0 ? (
          <p className="mt-2 text-sm text-[--text-secondary]">No backup metadata saved yet.</p>
        ) : (
          <ul className="mt-3 space-y-2 text-sm text-[--text-secondary]">
            {existingBackups.map((record) => (
              <li key={record.credentialHash} className="rounded-lg border border-[--border-default] bg-[--bg-elevated] px-3 py-2">
                <p className="font-mono text-xs">{record.credentialHash}</p>
                <p className="text-xs">{record.label ?? "Backup credential"}</p>
                <p className="text-xs">Saved {new Date(record.updatedAt).toLocaleString()}</p>
              </li>
            ))}
          </ul>
        )}
