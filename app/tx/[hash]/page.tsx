      ...
      <main className="min-h-screen bg-[--bg-base] px-4 py-10 sm:px-6 sm:py-16">
        <div className="mx-auto w-full max-w-2xl space-y-5">
          <div className="space-y-2 rounded-2xl border border-[--status-error-border] bg-[--status-error-bg] p-4 sm:p-6">
            <p className="text-xl font-semibold tracking-tight text-[--status-error-text]">
          ...
          <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
            ...
            <a
              ...
              className="inline-flex h-11 w-full items-center justify-center rounded-[--radius-lg] border border-[--border-glass] bg-[--bg-glass] backdrop-blur-md shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] px-5 text-sm font-semibold text-[--text-primary] transition-all duration-200 hover:bg-[--bg-glass-hover] hover:border-[--border-glass-hover] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[--brand-primary]/60 focus-visible:ring-offset-2 sm:w-auto"
            >
              Open Explorer Home
            </a>
          </div>
...
function TxDetailView({ hash }: { hash: `0x${string}` }) {
  ...
  const decoded = useMemo(() => {
    if (
      !tx?.to ||
      tx.to.toLowerCase() !== PATHUSD_ADDRESS.toLowerCase() ||
      !tx.input
    ) {
    ...
    try {
      ...
      const memo = hexToString(memoRaw as `0x${string}`, { size: 32 }).replace(
        /\0+$/g,
        "",
      );
      return {
    ...
    <main className="min-h-screen bg-[--bg-base] px-4 py-10 sm:px-6 sm:py-16">
      <div className="mx-auto w-full max-w-2xl space-y-5">
        {txLoading || receiptLoading ? (
          <div className="flex items-center gap-3 rounded-[--radius-xl] border border-[--border-glass] bg-[--bg-glass] backdrop-blur-md shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] p-4 text-sm text-[--text-tertiary] sm:p-6">
          ...
          <TxStatus
            ...
            status={status}
            blockNumber={
              receipt?.blockNumber ? receipt.blockNumber.toString() : undefined
            }
