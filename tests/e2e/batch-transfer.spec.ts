...
test.describe("Batch Transfer", () => {
  ...
  test("batch send supports multi-recipient review flow on authenticated dashboard", async ({
    page,
  }) => {
    test.skip(
    ...
    await expect(
      page.getByText("Confirm Batch Send", { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", { name: "Review Batch" }),
    ).toBeVisible();
    await expect(recipientInputs).toHaveCount(2);
