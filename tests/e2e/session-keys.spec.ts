...
async function navigateToDashboardWithMockWallet(
  page: Page,
  userAddress: string,
) {
...
test.describe("Session Keys", () => {
  ...
  test("auth baseline redirects unauthenticated /app to landing", async ({
    page,
  }) => {
    await page.goto("/app");
    ...
    await expect(
      page
        .getByRole("button", {
          name: /Create( Your)? Wallet|Create New Wallet/i,
        })
        .first(),
    ).toBeVisible();
  });

  test("creates and revokes a session key without localStorage session keys", async ({
    page,
  }) => {
    test.skip(
      !RUN_DASHBOARD_SCENARIOS,
      "Requires passkey auth to successfully reach /app dashboard.",
    );
    ...
    const sessionCard = page
      .locator("section")
      .filter({ has: page.getByText("Session Keys", { exact: true }) });
    await expect(sessionCard).toBeVisible();

    await sessionCard.getByRole("button", { name: "Create Session" }).click();
    await expect(
      sessionCard.getByText("Active Session", { exact: true }),
